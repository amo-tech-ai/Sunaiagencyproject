---
task_id: 002-WIZ
title: Wizard Step 2 — Industry Diagnostics
phase: CRITICAL
priority: P0
status: In Progress
...
area: wizard
wizard_step: 2
skill: [design/frontend-design, product/feature-dev, ai/gemini, data/supabase-edge-functions]
subagents: [supabase-expert]
edge_function: generate-diagnostics
schema_tables: [wizard_answers, ai_cache, industry_playbooks]
depends_on: [001-WIZ]
figma_prompt: prompts/02-industry-diagnostics.md
mermaid_diagram: mermaid-wizard/03-industry-diagnostics.md
---
...
## Screen Purpose
Diagnose the client's pain points and opportunities with industry-specific questions. Each question is designed to reveal a signal that maps to a system recommendation in Step 3. The right panel educates the user on WHY each question matters — building trust and demonstrating expertise.
...
The left panel is a 240px-wide vertical sidebar with two main sections separated by a horizontal divider.

**Top section — Progress Stepper:**
- The heading "DISCOVERY WIZARD" appears at the top of the panel.
- Below it, a vertical step list:
  - Step 1 "Business Context" — shown as completed with a green checkmark icon.
  - Step 2 "Industry Analysis" — shown as the currently active step with a filled green dot.
  - Step 3 — shown as incomplete with an empty circle.
  - Step 4 — shown as incomplete with an empty circle.
  - Step 5 — shown as incomplete with an empty circle.

**Bottom section — Detected Context:**
- Below a horizontal divider, the heading "DETECTED CONTEXT" appears in caps.
- Four labeled fields are stacked vertically, each showing a label and value:
  - Company: Acme Corp
  - Industry: E-commerce
  - Size: Medium (21-100)
  - Goal: Growth

### Center Panel — Diagnostic Questions

The center panel is a flexible-width column (max 640px) containing the page heading and 8 diagnostic questions divided into two sections.

**Page heading:**
- Title: "Industry Analysis" in Playfair Display at 32px.
- Subtitle: "Targeted questions to understand your operations, customers, and gaps." in Lora at 16px, muted color.

**Section 1 — GENERAL (section divider label):**

- **Question 1: "How do customers currently find you?" (required)**
  - Input type: Multi-select card grid, 3 columns by 2 rows. Each card is a checkbox-style selectable tile.
  - Options: Google Search, Social Media, Referrals, Paid Ads, Walk-in, Other.
  - Helper text below: "Select all that apply."

- **Question 2: "What does your sales process look like today?" (required)**
  - Input type: Dropdown select (single choice).
  - Options: Fully manual, Partially automated, Mostly automated, Fully automated.

- **Question 3: "How do you handle customer support inquiries?" (required)**
  - Input type: Dropdown select (single choice).
  - Options: Email only, Phone, Live chat, Chatbot, No formal process.

- **Question 4: "What tools/software do you currently use?" (required)**
  - Input type: Multi-select card grid, 3 columns by 2 rows.
  - Options: CRM, Email Marketing, Analytics, Social Media, Accounting, None.

**Section 2 — E-COMMERCE SPECIFIC (section divider label):**

- **Question 5: "What is your average cart abandonment rate?" (required)**
  - Input type: Dropdown select (single choice).

- **Question 6: "How do you handle post-purchase follow-up?" (required)**
  - Input type: Dropdown select (single choice).

- **Question 7: "Do you use personalized product recommendations?" (required)**
  - Input type: Single-select card row, 3 options side by side.
  - Options: Yes, No, Partially.

- **Question 8: "What % of revenue comes from repeat customers?" (required)**
  - Input type: Dropdown select (single choice).

### Right Panel — Dynamic Context

The right panel is 320px wide and its content changes based on which question the user is currently focused on. It contains three stacked sections separated by horizontal dividers.

**Section 1 — WHY THIS QUESTION:**
- Heading: "WHY THIS QUESTION" in 12px uppercase, muted color.
- Below the heading, a bordered card contains a short paragraph of contextual explanation that updates dynamically as the user focuses on each question. For example, when the user focuses on the cart abandonment question, the card reads: "Cart abandonment above 50% signals a high-value recovery opportunity. Automated follow-up sequences typically recover 10-15% of abandoned carts."

**Section 2 — SIGNALS DETECTED:**
- Heading: "SIGNALS DETECTED" in uppercase.
- A bordered card containing a vertical list of signal pills that appear dynamically as the user answers questions. Each signal is prefixed with a filled green dot. Example signals:
  - Low automation maturity
  - No CRM system
  - High cart abandonment
- At the bottom of the card, a counter reads (e.g.) "3 opportunities identified."

**Section 3 — INDUSTRY BENCHMARK:**
- Heading: "INDUSTRY BENCHMARK" in uppercase.
- A bordered card containing relevant industry statistics. For the e-commerce example: "Average e-commerce cart abandonment: 69.8%" and "Top performers: under 40%."
...
| Question | Answer Signal | Maps to System |
|----------|---------------|----------------|
| Customers find you: only 1-2 channels | Channel dependency risk | Growth Engine |
| Sales process: fully manual | Automation opportunity | Sales Automation |
| Support: email only / no process | Support gap | AI Support Chatbot |
| Tools: no CRM | CRM needed | CRM System |
| Cart abandonment: >50% | Recovery opportunity | Cart Recovery System |
| Post-purchase: nothing/manual | Retention gap | Follow-Up Automation |
| No personalization | Personalization opportunity | AI Recommendation Engine |
| Repeat customers: <20% | Loyalty gap | Loyalty & Retention System |

---

## "Why This Question" Content (per question)

**Q1 — Customer Channels:**
"Understanding how customers find you reveals channel dependency. Businesses relying on a single channel face higher risk and miss opportunities for AI-driven acquisition."

**Q2 — Sales Process:**
"Manual sales processes cap your growth. Partial automation shows readiness for AI-assisted sales. We calibrate our recommendations to your current maturity."

**Q3 — Support:**
"Support inquiries are the #1 opportunity for AI automation. Chatbots handle 60-80% of common questions, freeing your team for complex issues."

**Q4 — Tools:**
"Your current tech stack determines integration complexity. Missing tools = greenfield opportunity. Existing tools = integration-first approach."

**Q5 — Cart Abandonment (E-commerce):**
"Cart abandonment above 50% signals a high-value recovery opportunity. Automated follow-up sequences typically recover 10-15% of abandoned carts."

**Q6 — Post-Purchase (E-commerce):**
"Post-purchase follow-up drives repeat revenue. Automated emails generate 3-5x more revenue per recipient than broadcast campaigns."

**Q7 — Personalization (E-commerce):**
"Personalized product recommendations increase average order value by 10-30%. This is one of the highest-ROI AI applications in e-commerce."

**Q8 — Repeat Customers (E-commerce):**
"Acquiring a new customer costs 5-7x more than retaining one. Low repeat rates suggest loyalty and retention systems can significantly reduce CAC."
...
### Tablet
- Left panel: collapses to top bar with context summary
- Questions: full width, single column
- Right panel: collapsible "Insights" drawer, slide from right

### Mobile
- Progress: "Step 2 of 5" top bar
- Questions: full width, stacked, 16px padding
- "Why This Question" becomes an info icon (i) next to each question label — tap to reveal tooltip
- Signals section: horizontal scroll pills below questions
- Multi-select cards: 2 columns on mobile
...
|-------|---------------|
| screen_id | step-2 |
| data.customer_channels | google_search, social_media, paid_ads |
| data.sales_process | partially_automated |
| data.support_method | email_only |
| data.current_tools | email_marketing, accounting |
| data.cart_abandonment | 50_70 |
| data.post_purchase_followup | nothing |
| data.uses_personalization | no |
| data.repeat_customer_pct | under_20 |
| data.detected_signals | low_automation_maturity, no_crm, high_cart_abandonment, no_retention_system |

---

## AI Processing (Background)

After Step 2 completion and before user reaches Step 3:
1. Edge Function triggered
2. Combines Step 1 + Step 2 data
3. Runs analysis against system catalog
4. Generates ranked recommendations with reasoning
5. Caches in `ai_cache` for instant Step 3 load
6. Uses `gemini-3-flash-preview` for speed

---

## Workflow

1. **User arrives at /app/wizard/step-2**
   - Load wizard_answers for step-1 to get the selected industry
   - Load the industry-specific question bank (universal Q1-Q4 plus industry Q5-Q8)
   - Load any saved step-2 answers for pre-fill

2. **User answers questions**
   - Each answer is debounced by 500ms, then saved to wizard_answers.data
   - Left panel "Signals Detected" updates based on answer patterns:
     - Example: cart_abandonment greater than 50% adds a "High cart abandonment" signal
     - Example: support equals "No formal process" adds a "No support structure" signal
   - Right panel "Why this matters" content updates when a question receives focus
   - Footer shows "X of 8 answered" count

3. **User clicks "Continue"**
   - Validate that all 8 questions are answered (or explicitly set to "Don't know")
   - Save final state
   - Trigger background Edge Function: analyze_diagnostics
     - Input: step-1 and step-2 answers
     - Output: ranked system recommendations, stored in ai_cache
   - Update wizard_sessions.current_step to 3
   - Navigate to step-3

4. **User clicks "Back"**
   - Navigate to step-1 (all data is preserved)
