...
flowchart TD
    ...
    subgraph S1["Step 1: Business Context"]
        ...
        S1_URL{"Website URL\nprovided?"}
        S1_AI["AI: analyze-business\nURL Context + Google Search\n(background, non-blocking)"]
...
sequenceDiagram
    ...
    Note over User,AI: Step 1 -- Business Context (URL Analysis)

    User->>WizardUI: Fill company info form
    User->>WizardUI: Enter website URL

    rect rgb(45, 95, 63)
        Note over WizardUI,AI: URL Analysis (background, non-blocking)
        WizardUI->>AI: analyze-business(url, session_id)
        Note over AI: Gemini with URL Context + Google Search
        AI->>AI: URL Context: read full website<br/>(HTML, images, PDFs up to 34MB)
        AI->>AI: Google Search: company context,<br/>competitors, industry, market data
        AI->>Supabase: UPSERT ai_cache<br/>(url-analysis:{session_id})
        AI-->>WizardUI: Company analysis result<br/>(industry, size, products,<br/>tech signals, AI opportunities)
        WizardUI-->>User: Right panel shows analysis<br/>+ auto-fill suggestions
    end
...
flowchart TB
    ...
    subgraph RIGHT["Right Panel Progression"]
        direction TB
        RP1["Step 1\nGuidance + Analysis\nField tips → URL analysis"] --> RP2["Step 2\nIntelligence Mode\nLive signal detection"]
        RP2 --> RP3["Step 3\nIntelligence Mode\nAI reasoning & ROI"]
...
|------|-------------|----------------|-----------------|------------|
| 0 | Auth / Welcome | -- | -- | None |
| 1 | Company info form | Progress indicator, timer | Guidance (field tips) → Company Analysis (after URL) | `analyze-business()` on URL blur (URL Context + Google Search) |
