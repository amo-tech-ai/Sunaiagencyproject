...
1. **`kv_store_283466b6`** — 0 RLS policies. Figma Make artifact, potential data exposure.
2. **`wizard_sessions`/`wizard_answers`** — 7 policies each (anon + auth). By design for pre-auth wizard, but verify anon policies are read-only.
3. **`systems`/`system_services`** — Only 2 SELECT policies. Correct for reference data, but verify no admin write needed.
