...
export function StepExecutiveSummary() {
  ...
  // Fetch AI-generated readiness score + brief content
  useEffect(() => {
    if (!sessionId) return;
    let cancelled = false;
    setAiLoading(true);

    aiApi.readinessScore(sessionId).then(({ data, error }) => {
      if (cancelled) return;
      if (data && !error) {
        setAiContent(data);
      }
    }).catch((e) => {
      console.warn('[StepExecutiveSummary] AI readiness score failed, using local content:', e);
    }).finally(() => {
  ...
  // Generate brief content — AI enriches when available, mock templates as fallback
  ...
  }), [step1, step3, step4.briefEdits, diagnosticSignals, selectedSystems, aiContent]);
          ...
          <div>
            ...
            {aiLoading && (
              <div className="mt-3 flex items-center gap-2">
                <RefreshCw className="w-3 h-3 animate-spin" style={{ color: '#00875A' }} />
                <span className="text-xs" style={{ color: '#00875A' }}>AI generating strategy brief...</span>
              </div>
            )}
          </div>
