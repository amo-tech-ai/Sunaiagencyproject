import { motion } from 'motion/react';
import { ArrowRight } from 'lucide-react';
import { useNavigate } from 'react-router';
import backgroundImg from 'figma:asset/fa251af8ed15b4eeb792fb384c36b25796354e5c.png';

export default function WebDesignHero() {
  const navigate = useNavigate();

  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      {/* Cinematic Blurred Background */}
      <div className="absolute inset-0">
        <img 
          src={backgroundImg} 
          alt="" 
          className="w-full h-full object-cover"
        />
        {/* Dark overlay for text contrast */}
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/50 to-transparent" />
      </div>

      <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-24 lg:py-32 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          {/* Left Column - Text Content */}
          <motion.div
            initial={{ opacity: 0, x: -40 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            className="max-w-2xl"
          >
            {/* Small Label */}
            <motion.div
              className="mb-6"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6 }}
            >
              <span className="text-xs uppercase tracking-[0.2em] text-white/80 font-semibold font-['Lora']">
                LANDING PAGE SYSTEMS
              </span>
            </motion.div>

            {/* Headline with Italic Emphasis */}
            <motion.h1
              className="text-5xl lg:text-7xl xl:text-8xl mb-8 text-white leading-[1.1]"
              style={{ fontFamily: 'Playfair Display, serif' }}
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.1 }}
            >
              High-converting landing pages,{' '}
              <span className="italic">engineered for performance</span>
            </motion.h1>

            {/* Supporting Paragraph */}
            <motion.p
              className="text-lg lg:text-xl text-white/90 mb-10 leading-relaxed font-['Lora'] max-w-xl"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.8, delay: 0.2 }}
            >
              Custom, AI-optimized landing pages designed for campaign velocity, conversion lift, and measurable growth.
              We combine UX architecture, automation, and intelligent data flows to turn traffic into revenue.
            </motion.p>

            {/* CTA Buttons */}
            <motion.div
              className="flex flex-col sm:flex-row gap-4 mb-8"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.3 }}
            >
              <button
                onClick={() => navigate('/booking')}
                className="bg-[#84CC16] text-gray-900 px-8 py-4 rounded-full font-['Lora'] font-semibold hover:bg-[#73b512] hover:shadow-lg hover:shadow-[#84CC16]/30 transition-all flex items-center justify-center gap-3 text-base"
              >
                Book a Strategy Call
                <ArrowRight className="w-5 h-5" />
              </button>

              <button
                onClick={() => navigate('/projects')}
                className="text-white px-8 py-4 font-['Lora'] font-medium hover:text-[#84CC16] transition-colors flex items-center justify-center gap-2 text-base"
              >
                See case studies
                <ArrowRight className="w-4 h-4" />
              </button>
            </motion.div>
          </motion.div>

          {/* Right Column - Device Mockups */}
          <motion.div
            initial={{ opacity: 0, x: 40 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 1, delay: 0.3 }}
            className="relative hidden lg:block"
          >
            <div className="relative">
              {/* Laptop Mockup */}
              <motion.div
                initial={{ opacity: 0, y: 40 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 1, delay: 0.5 }}
                className="relative z-10"
              >
                {/* Laptop Frame */}
                <div className="bg-gray-900 rounded-t-lg shadow-2xl border-2 border-gray-800">
                  {/* Screen Content */}
                  <div className="bg-gradient-to-br from-[#0F3D3E] to-[#1a5f5e] p-12 rounded-t-lg aspect-video flex flex-col justify-center items-start">
                    <div className="text-white">
                      <h3 className="font-['Playfair_Display'] text-4xl mb-4">
                        Your AI-powered growth system
                      </h3>
                      <p className="font-['Lora'] text-white/70 text-sm mb-6">
                        Conversion tracking. CRM sync. Automated follow-ups.
                      </p>
                      
                      {/* Metrics Grid */}
                      <div className="grid grid-cols-3 gap-4 mt-8">
                        <div className="bg-white/10 backdrop-blur-sm p-3 rounded">
                          <div className="text-[#84CC16] font-bold text-xl">94%</div>
                          <div className="text-white/60 text-xs">Score</div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-sm p-3 rounded">
                          <div className="text-[#84CC16] font-bold text-xl">2.1s</div>
                          <div className="text-white/60 text-xs">Load</div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-sm p-3 rounded">
                          <div className="text-[#84CC16] font-bold text-xl">67%</div>
                          <div className="text-white/60 text-xs">CVR</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  
                  {/* Laptop Bottom */}
                  <div className="bg-gray-800 h-3 rounded-b-lg" />
                </div>
                
                {/* Laptop Base */}
                <div className="bg-gradient-to-b from-gray-700 to-gray-800 h-2 w-full mx-auto rounded-b-xl shadow-lg" />
              </motion.div>

              {/* Phone Mockup - Overlapping Bottom Right */}
              <motion.div
                initial={{ opacity: 0, y: 60, x: -20 }}
                animate={{ opacity: 1, y: 0, x: 0 }}
                transition={{ duration: 1, delay: 0.7 }}
                className="absolute -bottom-16 -right-8 z-20"
              >
                {/* Phone Frame */}
                <div className="bg-gray-900 rounded-[2.5rem] shadow-2xl border-[6px] border-gray-800 w-64">
                  {/* Notch */}
                  <div className="bg-gray-900 h-6 w-32 mx-auto rounded-b-2xl relative -top-[6px]" />
                  
                  {/* Screen Content */}
                  <div className="bg-white p-6 -mt-6 rounded-[2rem] min-h-[520px] flex flex-col">
                    {/* Header */}
                    <div className="mb-6">
                      <div className="flex items-center gap-2 mb-4">
                        <div className="w-8 h-8 bg-[#84CC16] rounded-full" />
                        <span className="font-['Playfair_Display'] text-gray-900 text-xl font-bold">otto</span>
                      </div>
                      <h4 className="font-['Playfair_Display'] text-2xl text-gray-900 mb-2">
                        Your AI-powered dept advisor
                      </h4>
                      <p className="text-gray-600 text-sm font-['Lora']">
                        Get personalized financial guidance in minutes
                      </p>
                    </div>

                    {/* Lead Capture Form */}
                    <div className="space-y-3 mb-6">
                      <input
                        type="text"
                        placeholder="Full name"
                        className="w-full px-4 py-3 bg-gray-100 rounded-lg text-sm"
                        disabled
                      />
                      <input
                        type="email"
                        placeholder="Email address"
                        className="w-full px-4 py-3 bg-gray-100 rounded-lg text-sm"
                        disabled
                      />
                    </div>

                    {/* CTA Button */}
                    <button
                      className="w-full bg-[#84CC16] text-gray-900 py-4 rounded-full font-['Lora'] font-semibold text-sm mb-4"
                      disabled
                    >
                      Get Started →
                    </button>

                    {/* Performance Badge */}
                    <div className="mt-auto pt-4 border-t border-gray-200">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-500 font-['Lora']">Powered by AI</span>
                        <div className="flex gap-2">
                          <span className="bg-[#84CC16]/10 text-[#84CC16] px-2 py-1 rounded font-semibold">Secure</span>
                          <span className="bg-[#84CC16]/10 text-[#84CC16] px-2 py-1 rounded font-semibold">Fast</span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </motion.div>
            </div>
          </motion.div>
        </div>
      </div>
    </section>
  );
}
