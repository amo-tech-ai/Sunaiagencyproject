# 08 — FLOW DIAGRAMS (MERMAID)
# Wizard Visual Architecture

**Status:** REFERENCE
**Parent Doc:** `00-wizard.md`
**Rendering:** Use any Mermaid-compatible viewer (GitHub, VS Code extension, mermaid.live)

---

## TABLE OF CONTENTS

1. [Complete Wizard Flow](#1-complete-wizard-flow)
2. [Wizard State Machine](#2-wizard-state-machine)
3. [Data Flow Overview](#3-data-flow-overview)
4. [AI Processing Pipeline](#4-ai-processing-pipeline)
5. [Three-Panel Layout Architecture](#5-three-panel-layout-architecture)
6. [Panel Content Transitions](#6-panel-content-transitions)
7. [Navigation & Back Flow](#7-navigation--back-flow)
8. [Simplified Step Flow](#8-simplified-step-flow)
...
All wizard states and their transitions, including error, resume, and back paths.
...
## 2B. DETAILED SESSION STATE MACHINE

Includes brief editing substates and AI processing.
...
## 6. PANEL CONTENT TRANSITIONS

How each panel transitions its content as the user progresses through steps.
...
## 7. NAVIGATION & BACK FLOW

Bidirectional navigation with auto-save persistence.
...
## 8. SIMPLIFIED STEP FLOW

Compact view with validation gates and AI triggers.
...
| Function | Trigger Point | Blocking? | Fallback |
|----------|--------------|-----------|----------|
| `analyze_diagnostics()` | Step 2 → Continue | No (background) | Rule-based signal extraction |
| `generate_recommendations()` | Called by `analyze_diagnostics` | No (pre-cached for Step 3) | Industry-to-system priority map |
| `generate_brief()` | Step 4 entry | Yes (skeleton loading, 5-10s) | Template-based brief from data |
| `generate_tasks()` | Step 5 project creation | Yes (background, non-blocking UI) | Template task list per system |

### Supabase Table Summary

| Table | Created At | Updated At | Key Fields |
|-------|-----------|-----------|------------|
| `wizard_sessions` | Step 0 (auth) | Every step | `current_step`, `status` |
| `wizard_answers` | Steps 1-2 | Auto-save (debounce 500ms) | `step_number`, `question_key`, `answer_value` |
| `wizard_recommendations` | Step 2→3 transition | Step 3 (user selects) | `system_id`, `fit_score`, `selected` |
| `wizard_briefs` | Step 4 entry | Step 4 (user edits) | `content_json`, `version`, `approved` |
| `projects` | Step 5 | Dashboard | `name`, `status`, `brief_snapshot` |
| `project_tasks` | Step 5 | Dashboard | `title`, `priority`, `status` |
