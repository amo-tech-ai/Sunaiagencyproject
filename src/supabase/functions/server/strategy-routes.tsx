...
// Uses adminClient for service-level access. Auth via requireAuth/getUserFromToken.
...
function errorResponse(c: any, context: string, error: unknown) {
  ...
  console.log(`[Strategy] ${context} exception (${status}): ${msg}`);
...
strategy.get(`${PREFIX}/strategy/canvas`, async (c) => {
  try {
    ...
    const { userId, error: authErr } = await getUserFromToken(authHeader ?? null);
    if (!userId) return c.json({ error: authErr || "Authentication required" }, 401);
    ...
    else if (userId !== "anonymous") query = query.eq("user_id", userId);
...
strategy.put(`${PREFIX}/strategy/canvas/:id`, async (c) => {
  try {
    ...
    const { data: current, error: fetchErr } = await db
      .from("lean_canvases").select("*").eq("id", canvasId).single();
...
strategy.get(`${PREFIX}/strategy/insights`, async (c) => {
  try {
    ...
    let query = db.from("strategy_insights").select("*").order("created_at", { ascending: false }).limit(limit);
...
strategy.put(`${PREFIX}/strategy/insights/:id`, async (c) => {
  try {
    ...
    const { data, error } = await db.from("strategy_insights").update(updates).eq("id", id).select().single();
...
strategy.get(`${PREFIX}/strategy/opportunities`, async (c) => {
  try {
    ...
    const db = adminClient();
    const { data, error } = await db
      .from("automation_opportunities").select("*")
...
strategy.put(`${PREFIX}/strategy/opportunities/:id`, async (c) => {
  try {
    ...
    const { data, error } = await db.from("automation_opportunities").update(updates).eq("id", id).select().single();
...
strategy.get(`${PREFIX}/strategy/recommendations`, async (c) => {
  try {
    ...
    let query = db.from("strategy_recommendations").select("*").order("created_at", { ascending: false });
...
strategy.post(`${PREFIX}/strategy/recommendations/:id/approve`, async (c) => {
  try {
    ...
    const { data: rec, error: fetchErr } = await db
      .from("strategy_recommendations").select("*").eq("id", id).single();
    ...
    const { data: updated, error: updateErr } = await db
      ...
      .update({
        ...
        approved_by: userId !== "anonymous" ? userId : null,
...
strategy.get(`${PREFIX}/strategy/metrics`, async (c) => {
  try {
    ...
    // Load canvas
    let canvasQuery = db.from("lean_canvases").select("*").eq("is_current", true);
    if (userId !== "anonymous") canvasQuery = canvasQuery.eq("user_id", userId);
    const { data: canvas } = await canvasQuery.order("created_at", { ascending: false }).limit(1).maybeSingle();

    // Load aggregation data
    const [insightsRes, oppsRes, recsRes, budgetRes] = await Promise.all([
      db.from("strategy_insights").select("*").order("created_at", { ascending: false }).limit(100),
      db.from("automation_opportunities").select("*"),
      db.from("strategy_recommendations").select("*"),
...
strategy.post(`${PREFIX}/strategy/analyze`, async (c) => {
  try {
    ...
    // 2. Load context
    const { data: canvas } = await db.from("lean_canvases").select("*").eq("id", canvas_id).single();
